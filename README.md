...
| `BUZZMINI_DONATE_URL` | URL страницы сбора (по умолчанию ВТБ Paymo в коде; переопределите для своей ссылки). |
...
Профиль на **Donatty** сейчас на проверке; когда он будет доступен, в окне доната и здесь появится и второй вариант. Свою ссылку для сборки можно задать переменной **`BUZZMINI_DONATE_URL`**.
